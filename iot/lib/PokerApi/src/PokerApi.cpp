...
bool PokerApi::sendCard(String firstCard, String secondCard) {
    ...
    JSONVar body;
    body["seat"] = SEAT_1;
    body["cardId"][0] = firstCard;
    body["cardId"][1] = secondCard;
...
void PokerApi::refreshSauvgarde() {
    this->chips->clear();
}

bool PokerApi::decidingSendChips(std::vector<String>* chips) {

    if (chips->size() != this->chips->size()) {
        this->refreshSauvgarde();
        for(int i = 0; i < chips->size(); i++) {
            this->chips->push_back(chips->at(i));
        }
...
bool PokerApi::decidingSendCards(String firstCard, String secondCard) {
    if ((firstCard != this->card1 || secondCard != this->card2) && (firstCard != "" && secondCard != "")) {

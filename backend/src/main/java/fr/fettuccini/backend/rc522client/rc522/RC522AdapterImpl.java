...
import static fr.fettuccini.backend.rc522client.rc522.RC522CommandTable.*;
import static fr.fettuccini.backend.rc522client.util.DataUtil.getStatus;
...
public class RC522AdapterImpl implements RC522Adapter {
	...
	public void init(int speed, int resetPin, int spiChannel) {
		if (speed < MIN_SPEED || speed > MAX_SPEED) {
			log.error("Speed out of range: {}. It should be between {} and {}", speed, MIN_SPEED, MAX_SPEED);
			return;
		}

		if (!raspberryPiAdapter.init(spiChannel, speed, resetPin)) {
			log.error("Error when initializing SPI channel");
		}
		reset();
	}
	...
	/**
	 * It selects a card and returns with the tagId. First it send a request, then
	 * it set up the anti-collision and
	...
	/**
	 * Authenticates a given block on your already selected card. You must select
	 * your card before this process!
	 *
	 * @param authMode     Either it can be PICC_AUTHENT1A or PICC_AUTHENT1B,
	 *                     depends on the key type, 1 byte long
	...
	/**
	 * Reads data from block. You must authenticate this block before reading from
	 * it! The returned CommunicationResult
	...
	/**
	 * Writes data to the target block. You must authenticate this block before
	 * writing to it!
	 *
	...
	private CommunicationResult antiCollision() {
		...
		if (result.isSuccess()) {
			if (result.getLength() == 5) {
				...
				log.error("backLen=" + result.getLength());
			}
		}
	...
	private void writeRC522(byte address, byte value) {
		...
		if (responseCode == -1) {
			log.error("Device SPI write error, address={}, value={}", address, value);
		}
	}
	...
	private byte readRC522(byte address) {
		...
		int responseCode = raspberryPiAdapter.wiringPiSPIDataRW(data);
		if (responseCode == -1) {
			log.error("Device SPI read error, address={}", address);
		}
